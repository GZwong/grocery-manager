...
def create_new_group():
    ...
    Input Request: Expects a JSON request
        {
            'name': 'group_name',
            'description': 'Example Description'  TODO: Take this to null until it has been changed
        }

    Returns status code, no content
    """
    try:
        data = request.json
        ...
        # Error if group name is not provided
        if not group_name:
        ...
        return jsonify({"status": "success", "message": "Group created successfully!"}), 200
